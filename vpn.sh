...
set_default_client_name () {
	client="client"
	suffix=2
	while [[ -e /etc/wireguard/wg0.conf ]] && grep -q "^# BEGIN_PEER $client$" /etc/wireguard/wg0.conf; do
...
new_client_setup () {
	...
	# DigitalOcean-friendly default: IPv4-only client peers.
	# This avoids accidental broken/slow IPv6 routing when the droplet or client network
	# does not have fully working IPv6 forwarding.
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
	...
	# If multiple IPv4 addresses exist, use the first one automatically.
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		...
		if [[ -z "$get_public_ip" ]]; then
			echo "Unable to detect public IPv4 address automatically."
			exit
		fi
	...
	# If multiple IPv6 addresses exist, use the first one automatically.
	...
	echo
	new_client_dns
	# Set up automatic updates for BoringTun automatically.
	if [[ "$is_container" -eq 0 ]]; then
		echo
...
EOF
	...
	# Enable forwarding and DigitalOcean-friendly TCP performance tuning.
	# BBR + fq can improve throughput and latency on many VPS networks.
